...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['add_movie'])) {
        ...
        $sql = "INSERT INTO movie2 (title, genre, release_year, director, note, poster) VALUES ('$title', '$genre', '$release_year', '$director', '$note', '$target_file')";
        $mysqli->query($sql);
        ...
        $sql = "UPDATE movie2 SET title='$title', genre='$genre', release_year='$release_year', director='$director', note='$note' WHERE id='$id'";
        $mysqli->query($sql);
    } elseif (isset($_POST['remove_movie'])) {
        $id = $_POST['id'];
        $sql = "DELETE FROM movie2 WHERE id='$id'";
        $mysqli->query($sql);
    }
}

// Fetch movies from the database
$result = $mysqli->query('SELECT * FROM movie2');
