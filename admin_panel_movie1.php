...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['add_movie'])) {
        ...
        $stmt = $mysqli->prepare("INSERT INTO movie1 (title, genre, release_year, director, note, poster) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssisss", $title, $genre, $release_year, $director, $note, $target_file);
        $stmt->execute();
        $stmt->close();
        ...
        $stmt = $mysqli->prepare("UPDATE movie1 SET title=?, genre=?, release_year=?, director=?, note=? WHERE id=?");
        $stmt->bind_param("ssissi", $title, $genre, $release_year, $director, $note, $id);
        $stmt->execute();
        $stmt->close();
    } elseif (isset($_POST['remove_movie'])) {
        $id = $_POST['id'];
        $stmt = $mysqli->prepare("DELETE FROM movie1 WHERE id=?");
...
$result = $mysqli->query('SELECT * FROM movie1');
